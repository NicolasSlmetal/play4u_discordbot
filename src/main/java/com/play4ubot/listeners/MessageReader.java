...
public class MessageReader extends ListenerAdapter {
    ...
    public void onMessageReceived(MessageReceivedEvent message) {
        ...
        } else if (isWait_answer().get(message.getGuild()) && message.getAuthor().getAsMention().equals(CmdDel.getMember().get(message.getGuild()))) {
            if (msg.contains("SIM")) {
                try {
                    this.getLoader().executeDangerousOperation(user_mention, message);
                }catch (IOException e){
                    message.getChannel().sendMessage(user_mention + ", não foi possível deletar a música :cry:").queue();
                }
                message.getChannel().sendMessage(user_mention + "**,Música deletada com sucesso**").queue();
                MessageReader.isWait_answer().replace(message.getGuild(), false);
            } else if (msg.contains("NÃO")) {
                message.getChannel().sendMessage(user_mention + ",Ok, não vou deletar").queue();
                MessageReader.isWait_answer().replace(message.getGuild(), false);
            } else {
